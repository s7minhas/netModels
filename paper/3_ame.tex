...
The importance of this assumption becomes clearer in the process of estimating a GLM via maximum likelihood. After having chosen a set of covariates and specifying a distribution, we construct the joint density function over all dyads using the observations from Table~\label{tab:canDesign} as an example.
...
In the following section we implement the approaches described here using an application chosen by \citet{cranmer:etal:2016}. In doing so, we will also highlight the benefits that the AME provides over alternatives such as ERGM and the latent distance model.
