...
\significancestatement{We review an additive and multiplicative (AME) effects model that can be used for conducting inference on cross-sectional and longitudinal networks. This method of analysis is distinctive in that it applies to data that include binary links as well as relations between actors that may be ordinal or Gaussian. Unlike alternative approaches, this approach can be easily a) interpreted as a regression, b) captures 1st, 2nd, and 3rd-order linkages, c) is computationally convenient, and d) out-performs alternative inferential approaches in terms of both predicting linkages and capturing network dependencies.}
