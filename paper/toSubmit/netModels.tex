...
\begin{abstract}
In recent years, we have seen the deveopment of a number of approaches to inferential network analysis. 

We introduce a latent factor approach that models interdependencies among observations using additive and multiplicative effects (AME) and can be applied to binary, ordinal, and continuous network data. We also compare and contrast this approach with alternative inferential approaches such as latent distance models and the exponential random graph model (ERGM).

Key difference between ERGMs and latent variable models in general is that you have to specify network terms

The latent distance model has proven to be useful and popular but the construction of the latent variable complicates inference on exogenous parameters
