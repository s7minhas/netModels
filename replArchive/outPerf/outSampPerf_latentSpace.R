...
################################################
# load mod results
if( !file.exists( paste0(resultsPath,'predData_outSample_latentSpace.rda') ) ){
	...
	predDfs = list( LSM=lsEuclPred,	'LSM (Bilinear)'=lsBilPred, 
		...
		'AME'=amePred )
	save(predDfs, file=paste0(resultsPath,'predData_outSample_latentSpace.rda'))	
	} else {
	load(paste0(resultsPath,'predData_outSample_latentSpace.rda'))
	}
################################################
...
for(ii in 1:length(sepPngList)){
	...
	yLo = yLo + .1 ; yHi = yHi + .1 }
tmp = tmp + annotate('text', hjust=0, x=.51, y=seq(0.05,0.45,.1), label=names(predDfs))
