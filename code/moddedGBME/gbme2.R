...
if(rd>0) {
 ...
 fit1_dyad<-glm(y~-1+x[,-(1:2)],family=fam)
 fit1_fixef<-glm(y~-1+as.factor(x[,1])+as.factor(x[,2]),family=fam)
}
if(rd==0){ fit1<-glm(y~-1+as.factor(x[,1])+as.factor(x[,2]),family=fam) }
                                                                                  
s<-fit1_fixef$coef[1:n]
r<-c(0,fit1_fixef$coef[n+1:(n-1)])
if(rd>0){
# beta.d<-fit1$coef[(2*n):length(fit1$coef)]               #beta.d
beta.d<-fit1_dyad$coef               #beta.d
        }
...
piS.bd<-as.matrix((n*(n-1))^2*summary(fit1_dyad)$cov.unscaled )
         }
