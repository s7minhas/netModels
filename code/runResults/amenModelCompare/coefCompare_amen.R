...
################################################
# load model results
load(paste0(resultsPath, 'ameFit_0.rda'))
...
load(paste0(resultsPath, 'ameFit_1.rda'))
...
load(paste0(resultsPath, 'ameFit_3.rda'))
...
load(paste0(resultsPath, 'ameFit_4.rda'))
...
print.xtable(
	...
	size='tiny', file=paste0(graphicsPath, 'regTable_ame.tex')
)
################################################
...
# summarize in q & d table
coefDfs = list(
	'AME SR (k=0)'=ameTab0,  'AME SR (k=1)'=ameTab1, 'AME SR (k=2)'=ameTab2,
	'AME SR (k=3)'=ameTab3, 'AME SR (k=4)'=ameTab4 )
...
# Add logit results
frame = insertCoefInfo(frame, model='AME SR (k=0)', error='int')
frame = insertCoefInfo(frame, model='AME SR (k=1)', error='int')
frame = insertCoefInfo(frame, model='AME SR (k=2)', error='int')
frame = insertCoefInfo(frame, model='AME SR (k=3)', error='int')
frame = insertCoefInfo(frame, model='AME SR (k=4)', error='int')
