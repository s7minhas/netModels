...
source(paste0(funcPath, 'coefHelpers.R'))
...
lapply(1:2, function(ii){
	# load ame results
	load(paste0(resultsPath, 'ameFitSR_',ii,'.rda'))
	...
	# data for plotting
	if(ii==1){toKeep = unique(ggConvData$variable)[2:6] ; fName = 'ameConv1_SR2.pdf'}
	if(ii==2){toKeep = unique(ggConvData$variable)[7:11] ; fName = 'ameConv2_SR2.pdf'}
