...
modsErgm = foreach(ii=1:length(nw.collabMiss) ) %dopar% {
...
# latentnet mod
cl=makeCluster(length(nw.collabMiss)) ; registerDoParallel(cl)
modsLs = foreach(ii=1:length(nw.collabMiss) ) %dopar% {
    source('~/Research/netModels/code/replicationSetup.R')
    load(paste0(dataPath, 'dvForCrossval.rda'))
...
cl=makeCluster(length(nw.collabMiss)) ; registerDoParallel(cl)
modsLsBil = foreach(ii=1:length(nw.collabMiss) ) %dopar% {
    source('~/Research/netModels/code/replicationSetup.R')
    load(paste0(dataPath, 'dvForCrossval.rda'))
...
# mrqap
cl=makeCluster(length(nw.collabMiss)) ; registerDoParallel(cl)
modsQap = foreach(ii=1:length(nw.collabMiss) ) %dopar% {
    source('~/Research/netModels/code/replicationSetup.R')
    load(paste0(dataPath, 'dvForCrossval.rda'))
